...
class FaissOrm:
    ...
    async def _ingest(
        ...
        documents = [
            Document(
                ...
                metadata=dict(id=id, collection=knowledge_base_name),
            )
    ...
    async def get_similarity(self, query: str, name: str) -> list[models.Document]:
        docs = await self.db.asimilarity_search_with_score(
            query, filter=dict(collection=name)
        )
        return [
            models.Document(id=doc.metadata["id"], content=doc.page_content)
            for doc, _ in docs
        ]
    ...
    def get_model() -> FAISS:
        ...
        )  # TODO: maybe we should preload this!
