...
@inject
async def new_knowledge_base(command: AddKnowledgeBase, bus: MessageBus= Depends(Provide[Container.knowledge_base_bus])) -> str:
    knowledge_base = await bus.handle(command)
    return knowledge_base.name


@router.delete(path='', name="Remove KnowledgeBase", status_code=status.HTTP_202_ACCEPTED)
@inject
async def remove_knowledge_base(command: RemoveKnowledgeBase, bus: MessageBus= Depends(Provide[Container.knowledge_base_bus])) -> str:
